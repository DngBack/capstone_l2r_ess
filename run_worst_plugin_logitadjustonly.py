#!/usr/bin/env python3
"""
Worst-group Plug-in with ONLY LogitAdjust expert (no gating) per "Learning to Reject Meets Long-Tail Learning"
=================================================================================================================

- Implements Algorithm 2 (Worst-group Plug-in) with inner Algorithm 1 (CS plug-in)
- Uses ONLY logitadjust expert logits (no gating)
...
- Expert logits: ./outputs/logits/cifar100_lt_if100/logitadjust_baseline/{split}_logits.pt

Outputs:
- results/ltr_plugin/cifar100_lt_if100/ltr_plugin_logitadjustonly_worst.json
- results/ltr_plugin/cifar100_lt_if100/ltr_rc_curves_balanced_worst_logitadjustonly_test.png
"""
...
def main():
    ...
    aurc_w = (
        float(np.trapz(ew_aurc, r_aurc)) if r_aurc.size > 1 else float(ew_aurc.mean() if ew_aurc.size else 0.0)
    )
    aurc_b = (
        float(np.trapz(eb_aurc, r_aurc)) if r_aurc.size > 1 else float(eb_aurc.mean() if eb_aurc.size else 0.0)
    )

    out_json = Path(CFG.results_dir) / "ltr_plugin_logitadjustonly_worst.json"
    ...
    plot_path = Path(CFG.results_dir) / "ltr_rc_curves_balanced_worst_logitadjustonly_test.png"
    plot_rc(r_plot, ew_plot, eb_plot, aurc_w, aurc_b, plot_path)
    # Plot Tail - Head gap curve
    gap_plot_path = Path(CFG.results_dir) / "ltr_tail_minus_head_worst_logitadjustonly_test.png"
    plt.figure(figsize=(7, 5))
    plt.plot(r_plot, gap_plot, "d-", color="crimson", label="Tail - Head error")
    plt.xlabel("Proportion of Rejections")
    plt.ylabel("Tail Error - Head Error")
    plt.title("Tail-Head Error Gap vs Rejection Rate (Worst LogitAdjust)")
