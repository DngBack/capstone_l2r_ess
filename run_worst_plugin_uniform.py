...
def main():
    ...
    aurc_w = (
        float(np.trapezoid(ew_aurc, r_aurc)) if r_aurc.size > 1 else float(ew_aurc.mean() if ew_aurc.size else 0.0)
    )
    aurc_b = (
        float(np.trapezoid(eb_aurc, r_aurc)) if r_aurc.size > 1 else float(eb_aurc.mean() if eb_aurc.size else 0.0)
    )
