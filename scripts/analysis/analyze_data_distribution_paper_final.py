...
def get_cifar100_lt_counts_paper(imb_factor=100):
    ...
    return counts  # KHÔNG normalize - giữ nguyên exponential profile
...
def create_longtail_test_val_sets_paper(cifar_test, train_class_counts, seed=42):
    """Tạo test/val sets - TÁCH ĐƠN GIẢN 20%/80% từ original test"""
    np.random.seed(seed)
    
    test_targets = np.array(cifar_test.targets)
    
    # Tách 20% làm validation, 80% làm test (ĐƠN GIẢN)
    val_size = int(0.2 * len(cifar_test))  # 20% của 10,000 = 2,000
    test_size = len(cifar_test) - val_size  # 80% của 10,000 = 8,000
    
    # Tách ngẫu nhiên từ test set
    all_indices = np.arange(len(cifar_test))
