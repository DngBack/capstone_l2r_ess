#!/usr/bin/env python3
"""
Worst-group Plug-in with ONLY BalSoftmax expert (no gating) per "Learning to Reject Meets Long-Tail Learning"
================================================================================================================

- Implements Algorithm 2 (Worst-group Plug-in) with inner Algorithm 1 (CS plug-in)
- Uses ONLY balsoftmax expert logits (no gating)
...
- Expert logits: ./outputs/logits/cifar100_lt_if100/balsoftmax_baseline/{split}_logits.pt

Outputs:
- results/ltr_plugin/cifar100_lt_if100/ltr_plugin_balonly_worst.json
- results/ltr_plugin/cifar100_lt_if100/ltr_rc_curves_balanced_worst_balonly_test.png
"""
...
class Config:
    ...
    # Use only balsoftmax expert
    expert_name: str = "balsoftmax_baseline"
...
def plot_rc(
    ...
    plt.plot(
        r_plot, ew_plot, "o-", label=f"Worst-group (AURC={aw:.4f})", color="royalblue"
    )
...
def main():
    ...
    expert_logits_s1 = load_expert_logits("tunev", DEVICE)
    labels_s1 = load_labels("tunev", DEVICE)
    expert_logits_s2 = load_expert_logits("val", DEVICE)
    ...
    post_s1 = F.softmax(expert_logits_s1, dim=-1)
    post_s2 = F.softmax(expert_logits_s2, dim=-1)
    post_test = F.softmax(expert_logits_test, dim=-1)
    ...
    aurc_w = (
        float(np.trapz(ew_aurc, r_aurc))
        if r_aurc.size > 1
        else float(ew_aurc.mean() if ew_aurc.size else 0.0)
    )
    aurc_b = (
        float(np.trapz(eb_aurc, r_aurc))
        if r_aurc.size > 1
        else float(eb_aurc.mean() if eb_aurc.size else 0.0)
    )

    out_json = Path(CFG.results_dir) / "ltr_plugin_balonly_worst.json"
    with open(out_json, "w", encoding="utf-8") as f:
        json.dump(
            {
                "description": f"Worst-group plug-in via Algorithm 2 with inner CS plug-in (Algorithm 1). Using ONLY {CFG.expert_name}.",
    ...
    if r_plot.size > 0 and r_plot[-1] < max_rejection_plot:
        ew_at_08 = float(np.interp(max_rejection_plot, r, ew))
        eb_at_08 = float(np.interp(max_rejection_plot, r, eb))
        gap_at_08 = float(np.interp(max_rejection_plot, r, gap))
        r_plot = np.append(r_plot, max_rejection_plot)
        ew_plot = np.append(ew_plot, ew_at_08)
        eb_plot = np.append(eb_plot, eb_at_08)
        gap_plot = np.append(gap_plot, gap_at_08)

    plot_path = Path(CFG.results_dir) / "ltr_rc_curves_balanced_worst_balonly_test.png"
    plot_rc(r_plot, ew_plot, eb_plot, aurc_w, aurc_b, plot_path)
    # Plot Tail - Head gap curve
    gap_plot_path = Path(CFG.results_dir) / "ltr_tail_minus_head_worst_balonly_test.png"
    plt.figure(figsize=(7, 5))
    plt.plot(r_plot, gap_plot, "d-", color="crimson", label="Tail - Head error")
    plt.xlabel("Proportion of Rejections")
    plt.ylabel("Tail Error - Head Error")
    plt.title("Tail-Head Error Gap vs Rejection Rate (Worst Bal)")
