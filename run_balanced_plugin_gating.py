...
class Config:
    ...
    expert_names: List[str] = field(default_factory=lambda: ["ce_baseline", "logitadjust_baseline", "balsoftmax_baseline"])
...
def load_gating_network(device: str = DEVICE) -> GatingNetwork:
...
def compute_mixture_posterior(
    expert_logits: torch.Tensor, gating_net: GatingNetwork, device: str = DEVICE
) -> torch.Tensor:
