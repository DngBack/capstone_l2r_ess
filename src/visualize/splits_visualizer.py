...
class SplitsDistributionVisualizer:
    ...
    def load_split_data(
        self, split_name: str, is_from_train: bool = True
        ...
        print(
            f"✅ Loaded {split_name}: {len(indices):,} samples, "
    ...
    def visualize_all_splits(self, save_path: Optional[str] = None):
        ...
        fig.suptitle(
            "CIFAR-100-LT Data Distribution Across All Splits",
            fontsize=16,
            fontweight="bold",
        )
        ...
        ax1.set_title(
            "All Splits - Per-Class Distribution", fontsize=14, fontweight="bold"
        )
        ax1.set_yscale("log")
        ...
        ax2 = axes[0, 1]
        train_splits = {
            k: v for k, v in splits_data.items() if k in ["train", "expert", "gating"]
        }
        ...
        ax2.set_title(
            "Training Splits - Long-Tail Distribution", fontsize=14, fontweight="bold"
        )
        ...
        ax3 = axes[1, 0]
        test_splits = {
            k: v for k, v in splits_data.items() if k in ["test", "val", "tunev"]
        }
        ...
        ax3.set_title(
            "Test Splits - Balanced Distribution", fontsize=14, fontweight="bold"
        )
    ...
    def visualize_expert_gating_comparison(self, save_path: Optional[str] = None):
        ...
        fig.suptitle(
            "Expert vs Gating Training Splits Comparison",
            fontsize=16,
            fontweight="bold",
        )
        ...
        ax2.set_title(
            "Normalized Distribution (Should Overlap)", fontsize=14, fontweight="bold"
        )
        ax2.grid(True, alpha=0.3)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Visualize CIFAR-100-LT splits distribution"
    )
    parser.add_argument(
        "--splits-dir",
        type=str,
